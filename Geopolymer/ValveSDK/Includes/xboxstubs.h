...
// 	char        abEnet[6];                      // Ethernet MAC address
// 	char        abOnline[20];                   // Online identification
// } XNADDR;
...
typedef struct {
	char        ab[8];                          // xbox to xbox key identifier
} XNKID;

typedef struct {
	char        ab[16];                         // xbox to xbox key exchange key
} XNKEY;

typedef struct _XSESSION_INFO
{
	XNKID sessionID;                // 8 chars
	XNADDR hostAddress;             // 36 chars
	XNKEY keyExchangeKey;           // 16 chars
