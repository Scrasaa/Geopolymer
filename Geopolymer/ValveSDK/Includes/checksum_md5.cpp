...
//  the data and converts chars into longwords for this routine.
// Input  : buf[4] - 
//			in[16] - 
// Output : static void
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Update context to reflect the concatenation of another buffer full of chars.
// Input  : *ctx - 
//			*buf - 
//			len - 
//-----------------------------------------------------------------------------
void MD5Update(MD5Context_t* ctx, unsigned char const* buf, unsigned int len)
{
    ...
    t = (t >> 3) & 0x3f;        /* chars already in shsInfo->data */
    ...
    /* Handle any remaining chars of data. */
    memcpy(ctx->in, buf, len);
}

//-----------------------------------------------------------------------------
// Purpose: Final wrapup - pad to 64-char boundary with the bit pattern 
...
void MD5Final(unsigned char digest[MD5_DIGEST_LENGTH], MD5Context_t* ctx)
{
    ...
    *p++ = 0x80;
    ...
    /* chars of padding needed to make 64 chars */
    count = 64 - 1 - count;

    /* Pad out to 56 mod 64 */
    if (count < 8)
    {
        /* Two lots of padding:  Pad the first block to 64 chars */
        memset(p, 0, count);
        //charReverse(ctx->in, 16);
        MD5Transform(ctx->buf, (unsigned int*)ctx->in);

        /* Now fill the next block with 56 chars */
        memset(ctx->in, 0, 56);
    }
    else
    {
        /* Pad block to 56 chars */
        memset(p, 0, count - 8);
    }
    //charReverse(ctx->in, 14);
    ...
    MD5Transform(ctx->buf, (unsigned int*)ctx->in);
    //charReverse((unsigned char *) ctx->buf, 4);
...
unsigned int MD5_PseudoRandom(unsigned int nSeed)
{
    ...
    return *(unsigned int*)(digest + 6);	// use 4 middle chars for random value
}
