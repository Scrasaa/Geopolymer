...
inline int Q_UnicodeConvertT(const SrcType* pIn, int nInChars, DstType* pOut, int nOutchars, EStringConvertErrorPolicy ePolicy)
{
	if (!pIn)
	{
		return 0;
	...
	{
		int nOutElems = nOutchars / sizeof(DstType);
		if (nOutElems <= 0)
			return 0;
...
// Decode one character from a UTF-8 encoded string. Treats 6-char CESU-8 sequences
// as a single character, as if they were a correctly-encoded 4-char UTF-8 sequence.
inline int Q_UTF8ToUChar32(const char* pUTF8_, uchar32& uValueOut, bool& bErrorOut)
{
	...
	// Expecting at least a two-char sequence with 0xC0 <= first <= 0xF7 (110...... and 11110...)
	...
	// 110..... two-char lead char
	if (!(uValue & (0x20 << 6)))
		goto decodeFinished;

	// Expecting at least a three-char sequence
	...
	// 1110.... three-char lead char
	if (!(uValue & (0x10 << 12)))
		goto decodeFinishedMaybeCESU8;

	// Expecting a four-char sequence, longest permissible in UTF-8
	...
	// 11110... four-char lead char. fall through to finished.
...
//-----------------------------------------------------------------------------
// Purpose: Perform conversion. Returns number of *chars* required if output pointer is NULL.
//-----------------------------------------------------------------------------
inline int Q_UTF8ToUTF16(const char* pUTF8, uchar16* pUTF16, int cubDestSizeInchars, EStringConvertErrorPolicy ePolicy = STRINGCONVERT_ASSERT_REPLACE)
{
	return Q_UnicodeConvertT< char, uchar16, true, Q_UTF8ToUChar32, Q_UChar32ToUTF16Len, Q_UChar32ToUTF16 >(pUTF8, 0, pUTF16, cubDestSizeInchars, ePolicy);
}