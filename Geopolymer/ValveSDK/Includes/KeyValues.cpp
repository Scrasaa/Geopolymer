...
{
	// Initialize destination buffer and state for conversion
	mbstate_t state = {};
	const wchar_t* src = unicode;

	// Convert wide characters (wchar_t) to multi-byte (UTF-8) using the safer version
	size_t result;
	errno_t err = wcsrtombs_s(&result, ansi, ansiBufferSize, &src, ansiBufferSize - 1, &state);
...
int UTF8ToUnicode(const char* ansi, wchar_t* unicode, int unicodeBufferSizeInBytes)
{
	...
	// Convert multi-byte (UTF-8) to wide characters (wchar_t) using the safer version
	size_t result;
	errno_t err = mbsrtowcs_s(&result, unicode, unicodeBufferSizeInBytes / sizeof(wchar_t), &src, (unicodeBufferSizeInBytes / sizeof(wchar_t)) - 1, &state);
...
T As(void* ptr)
{
	// Cast the input pointer (e.g., signature or function pointer) to type T
...
bool KeyValues::LoadFromBuffer(char const* resource_name, const char* buffer, void* file_system, const char* path_id)
{
	...
	// Cast the global function pointer to the correct function pointer type using the As function
	auto function = As<Fn>((void*)globals::Sig_KeyValue_LoadFromBuffer);

	return function(this, resource_name, buffer, file_system, path_id);
}
...
void KeyValues::Initialize(char* name)
{
	...
	auto function = As<Fn>((void*)globals::Sig_KeyValue_Intialize);
...
KeyValues* KeyValues::FindKey(const char* keyName, bool bCreate)
{
	...
	auto function = As<Fn>((void*)globals::Sig_KeyValue_FindKey);

	return function(this, keyName, bCreate);
}
