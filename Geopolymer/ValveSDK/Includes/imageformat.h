...
struct BGR565_t
{
	...
	unsigned short g : 6;		//  char order of output to 32 bit
	unsigned short r : 5;
...
{
	unsigned short b : 5;		// order of names changes
	unsigned short g : 5;		//  char order of output to 32 bit
	unsigned short r : 5;
	unsigned short a : 1;
...
struct BGRA4444_t
{
	...
	unsigned short g : 4;		//  char order of output to 32 bit
	unsigned short r : 4;
	unsigned short a : 4;
...
namespace ImageLoader
{
	...
	int  GetMipMapLevelcharOffset(int width, int height, enum ImageFormat imageFormat, int skipMipLevels);
	...
	// Strides denote the number of chars per each line, 
	// by default assumes width * # of chars per pixel
	...
	void charSwapImageData(unsigned char* pImageData, int nImageSize, ImageFormat imageFormat, int width = 0, int stride = 0);
	...
	//-----------------------------------------------------------------------------
	// Gets the size of the image format in chars
	//-----------------------------------------------------------------------------
	inline int SizeInchars(ImageFormat fmt)
	{
		return ImageFormatInfo(fmt).m_Numchars;
	}
